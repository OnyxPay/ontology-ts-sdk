...
describe("test transfer asset", () => {
  ...
  const sm2Account = {
    ...
    isDefault: false,
    publicKey:
      "1314031220580679fda524f575ac48b39b9f74cb0a97993df4fac5798b04c702d07a39",
    signatureScheme: "SM3withSM2"
  };
  test(
    "test_transfer_asset_ONT",
    async () => {
      ...
      const response = await socketClient.sendRawTransaction(
        tx.serialize(),
        false,
        true
      );
  ...
  test(
    "test transfer assets with sm2 account",
    async () => {
      const sm2PriEnc = new PrivateKey(
        sm2Account.key,
        KeyType.SM2,
        new KeyParameters(CurveLabel.SM2P256V1)
      );
      const sm2Pri = sm2PriEnc.decrypt(
        "123456",
        new Address(sm2Account.address),
        sm2Account.salt
      );
      console.log("pri: " + sm2Pri.key);
      const tx = makeTransferTx(
        "ONT",
        new Address("ATk57i8rMXFSBpHAdX3UQ4TNe48BBrfCoc"),
  ...
  test(
    "test transfer with multi assign address",
    async () => {
      ...
      const w = [
        {
          name: "MyWallet",
          version: "1.1",
          scrypt: { p: 8, n: 16384, r: 8, dkLen: 64 },
          identities: null,
          accounts: [
            {
              ...
              "enc-alg": "aes-256-gcm",
              key:
                "YfOr9im4rOciy3cV7JkVo9QCfrRT4IGLa/CZKUJfL29pM6Zi1oVEM67+8MezMIro",
              algorithm: "ECDSA",
              salt: "RCIo60eCJAwzkTYmIfp3GA==",
          ...
          accounts: [
            {
              ...
              "enc-alg": "aes-256-gcm",
              key:
                "gpgMejEHzawuXG+ghLkZ8/cQsOJcs4BsFgFjSaqE7SC8zob8hqc6cDNhJI/NBkk+",
              algorithm: "ECDSA",
              salt: "tuLGZOimilSnypT91WrenQ==",
              parameters: { curve: "P-256" },
              label: "",
              publicKey:
          ...
          accounts: [
            {
              ...
              "enc-alg": "aes-256-gcm",
              key:
                "guffI05Eafq9F0j3/eQxHWGo1VN/xpeIkXysEPeH51C2YHYCNnCWTWAdqDB7lonl",
              algorithm: "ECDSA",
              salt: "oZPg+5YotRWStVsRMYlhfg==",
              parameters: { curve: "P-256" },
              label: "",
              publicKey:
          ...
          accounts: [
            {
              ...
              "enc-alg": "aes-256-gcm",
              key:
                "fAknSuXzMMC0nJ2+YuTpTLs6Hl5Dc0c2zHZBd2Q7vCuv8Wt97uYz1IU0t+AtrWts",
              algorithm: "ECDSA",
              salt: "0BVIiUf46rb/e5dVZIwfrg==",
              parameters: { curve: "P-256" },
              label: "",
              publicKey:
          ...
          accounts: [
            {
              ...
              "enc-alg": "aes-256-gcm",
              key:
                "IufXVQfrL3LI7g2Q7dmmsdoF7BdoI/vHIsXAxd4qkqlkGBYj3pcWHoQgdCF+iVOv",
              algorithm: "ECDSA",
              salt: "zUtzh0B4UW0wokzL+ILdeg==",
              parameters: { curve: "P-256" },
              label: "",
              publicKey:
          ...
          accounts: [
            {
              ...
              "enc-alg": "aes-256-gcm",
              key:
                "PYEJ1c79aR7bxdzvBlj3lUMLp0VLKQHwSe+/OS1++1qa++gBMJJmJWJXUP5ZNhUs",
              algorithm: "ECDSA",
              salt: "uJhjsfcouCGZQUdHO2TZZQ==",
              parameters: { curve: "P-256" },
              label: "",
              publicKey:
          ...
          accounts: [
            {
              ...
              "enc-alg": "aes-256-gcm",
              key:
                "ZG/SfHRArUkopwhQS1MW+a0fvQvyN1NnwonU0oZH8y1bGqo5T+dQz3rz1qsXqFI2",
              algorithm: "ECDSA",
              salt: "6qiU9bgK/+1T2V8l14mszg==",
              parameters: { curve: "P-256" },
              label: "",
              publicKey:
      ...
      for (const v of w) {
        ...
        pris.push(
          p.decrypt(
            "1",
            new Address(v.accounts[0].address),
            v.accounts[0].salt,
            params
          )
        );
      ...
      const tx = makeTransferTx(
        "ONT",
        mulAddr,
        new Address("AazEvfQPcQ2GEFFPLF1ZLwQ7K5jDn81hve"),
        100,
        gasPrice,
        gasLimit,
        mulAddr
  ...
  test(
    "test get ongs to withdraw with rest api",
    async () => {
      const addr = adminAddress;
      const result = await restClient.getAllowance(
        "oxg",
  ...
  test("sort_pk", () => {
    const pk1 = new PublicKey(
      "03a3c7a40461238a210d306ce4a79db69800449173e47b9e2fa92b7815d7517872"
    );
    const pk2 = new PublicKey(
      "023c5b6e0e4fe8647d1065ecd09c60d251e1e168999202423e3be5d174866f9349"
    );
    const pks = [pk1, pk2];
    console.log(pks);
